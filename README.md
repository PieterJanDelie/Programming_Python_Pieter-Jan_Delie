# Examen python Basketball Stat Tracker
...
Deze terminalapplicatie, genaamd "Basketball Stat Tracker", is ontworpen om de statistieken van basketballspelers en wedstrijden bij te houden. Het biedt functionaliteiten voor het beheren van spelersgegevens, het invoeren van matchstatistieken en het exporteren van gemiddelde statistieken naar CSV- of Excel-bestanden.
...
2. Navigeer door het menu:

    De applicatie presenteert een menu met verschillende opties. Gebruik de cijfers om de gewenste actie te kiezen.

3. Functionaliteiten:

    - Speler Toevoegen (Optie 1): Voeg nieuwe basketballspelers toe aan de database.
    - Alle Spelers Tonen (Optie 2): Toon een lijst met alle geregistreerde spelers.
    - Matchstatistieken Toevoegen (Optie 3): Voeg statistieken toe voor een specifieke speler en wedstrijd.
    - Speler Bewerken (Optie 4): Bewerk de gegevens van een bestaande speler.
    - Speler Verwijderen (Optie 5): Verwijder een speler uit de database.
    - Matchstatistieken van Speler Tonen (Optie 6): Toon alle matchstatistieken van een specifieke speler.
    - Alle Statistieken van Match Tonen (Optie 7): Toon alle statistieken van een specifieke wedstrijd.
    - Gemiddeldes naar Extern Bestand Schrijven (Optie 8): Schrijf gemiddelde statistieken naar een CSV- of Excel-bestand.
    - Programma Afsluiten (Optie 9): Sluit de applicatie af.

## Structuur

- main.py: De hoofdapplicatie waarin de gebruikersinterface is geïmplementeerd.
- Controllers/DatabaseController.py: Bevat de `DatabaseController`-klasse voor het beheren van de SQLite-database en het uitvoeren van databasebewerkingen.
- Models/Speler.py: Definieert de `Speler`-klasse voor het modelleren van spelersgegevens.
- Models/MatchStats.py: Definieert de `MatchStats`-klasse voor het modelleren van matchstatistieken.
- Test/DatabaseTest.py: Definieert alle testen in verband met de database.
- Test/MainTest.py: Definieert alle testen in verband met de main file.

## Database

De applicatie maakt gebruik van een SQLite-database die twee tabellen bevat:

1. spelers: Bevat informatie over basketballspelers (nummer, voornaam, familienaam).
2. matchstats: Bevat matchstatistieken van spelers (ID, speler_nummer, matchnummer, gespeelde_minuten, gescoorde_punten).

Deze tabbelen zijn hebben een relatie door nummer van speler met speler_nummer van matchstats
